    ...
    "element": (
      <span>
        ...
        <SettingGroup name="Sidebar">
          ...
          <Setting name="Icon Size" setting="sidebar-icon-size" description="Increase or decrease the size of sidebar icons." defaultValue={5} customInput={(state, setState) => { return <Slider state={state} setState={setState} min={1} max={10}  defaultval="5" bubbleRenderer={(bubble) => { if(bubble === 5) return 'Normal'; else if (bubble > 5) return 'Large'; else if(bubble < 5) return "Small"; }} />}} />
