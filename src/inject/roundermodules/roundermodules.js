const onloadmutation = (el) => new MutationObserver(function(mutations) {
    ...
    onChanged: (data) => {
        if(data) {
            if (Array.from(document.querySelectorAll('span.ellipsible')).find(el => el.textContent === 'Assignments')) {
                ...
                onloadmutation(document.getElementById('ag-list')).observe(document.getElementById('ag-list'), { childList: true });
