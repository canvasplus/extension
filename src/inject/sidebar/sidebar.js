...
useReactiveFeatures([{
  ...
  onChanged: (value) => {
    sidebarIconWidth.setRule(value === 5 ? '' : '--sidebar-icon-width: ' + String((value * 2) + 16) + 'px');
  }
}, {
...
const renderSidebarButtons = () => {
  delayedQuerySelector("#header .ic-app-header__main-navigation #menu.ic-app-header__menu-list #global_nav_conversations_link").then((element) => {
    ...
    document.querySelectorAll(".sidebar-custom-menu-icon").forEach(element => {
      element.remove()
    })

    if(settingsButtonEnabled) {
      const settings = document.createElement('li')
      settings.className = 'menu-item ic-app-header__menu-list-item sidebar-custom-menu-icon'
    ...
    if(expansionDrawerEnabled) {
      ...
      more.className = 'menu-item ic-app-header__menu-list-item cpx-outside-settings-drawer sidebar-custom-menu-icon'
      more.id = 'sidebar-custom-menu-icon-more'
      more.innerHTML = '<button id="global_nav_settings_link" class="ic-app-header__menu-list-link"><div class="menu-item-icon-container" aria-hidden="true"><svg class="ic-icon-svg menu-item__icon" style="width: calc(var(--sidebar-icon-width, 26px) * 1.2) !important;" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1726.67 348.06"><g id="Layer_2" data-name="Layer 2"><circle cx="863.33" cy="174.03" r="174.03"/><circle cx="1552.64" cy="174.03" r="174.03"/><circle cx="174.03" cy="174.03" r="174.03"/></g></svg></div><div class="menu-item__text">More</div></button>'
...
const renderSidebarDrawer = () => {
  ...
  delayedQuerySelector("#menu").then(element => {
    ...
    document.querySelectorAll(".cpx-outside-settings-drawer:not(#sidebar-custom-menu-icon-more)").forEach(element => {
    ...
    chrome.storage.local.get(["canvasplus-setting-sidebar-drawer-all-items"], (data) => {
      if(data["canvasplus-setting-sidebar-drawer-all-items"]) {
        for(let item of data["canvasplus-setting-sidebar-drawer-all-items"]) {
          if(!allItems.includes(item)) allItems.push(item)
        }
      }
  
      chrome.storage.local.set({"canvasplus-setting-sidebar-drawer-all-items": allItems})
    });
  })
}
